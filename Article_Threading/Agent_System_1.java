...
import Article_Process.ArticleWriter;
...
public class Agent_System_1 implements Runnable {
  ...
  public void run()
  {
    ...
    if(Agent_System_1.messagesCompleted == true)
    {
      ...
      try {
        FileWriter productWriterFinalizer = ArticleWriter.get_file_writer(); 
        FileReader moduleReading = new FileReader("module_reading.txt");
        int moduleNumber = 10; 
        for(int i = 0; i < moduleNumber; i++) {
          Article.startCrossCheck(0, productWriterFinalizer, moduleReading);
        }
        moduleReading.close(); 
