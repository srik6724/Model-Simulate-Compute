...
public class Interrogative_Word_Queue<T> {
	//private Match_List<Object> matches = new Match_List<Object>();
	public static void main(String[]args) throws InterruptedException, IOException
	{
		Predicate<Path> criteria = path -> {
			String fileName = path.getFileName().toString(); 
			return fileName.endsWith(".txt") && (fileName.contains("cross_check") || fileName.contains("interrogative_word") || fileName.contains("category") || fileName.contains("token")); 
		}; 

		Predicate<Path> fileCountCriteria = path -> { 
			String fileName = path.getFileName().toString(); 
			return fileName.contains("cross_check"); 
		}; 
		
		Path startPath = Paths.get("C:/Users/srik6/OneDrive/Desktop/Interrogative-Word-Build-(1-1)"); 
		...
		Interrogative_Word assembly = new Interrogative_Word(module_reading); 
		assembly.enroll2Teams(args); 

		Path directory = Paths.get("C:/Users/srik6/OneDrive/Desktop/Interrogative-Word-Build-(1-1)");
